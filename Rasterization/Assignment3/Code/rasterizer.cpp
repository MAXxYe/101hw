...
void rst::rasterizer::draw(std::vector<Triangle *> &TriangleList) {
    ...
    {
        Triangle newtri = *t;
        // Array with 3 elements and each element is Vector4f    
        std::array<Eigen::Vector4f, 3> mm {
                (view * model * t->v[0]),
                (view * model * t->v[1]),
                (view * model * t->v[2])
        };// Do mvp transformation to every triangle vertex
        // 
        std::array<Eigen::Vector3f, 3> viewspace_pos;
        // copy first three elements of mm to viewspace_pos
