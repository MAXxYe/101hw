...
inline Intersection Triangle::getIntersection(Ray ray)
{
    ...
    if (dotProduct(ray.direction, normal) > 0)
        return inter;
    double u, v, t_tmp = 0;
    Vector3f pvec = crossProduct(ray.direction, e2); // pvec = P
    double det = dotProduct(e1, pvec); //
    if (fabs(det) < EPSILON)  // doesn't hit
        return inter;

    double det_inv = 1. / det;
    Vector3f tvec = ray.origin - v0; // tvec = T
    u = dotProduct(tvec, pvec) * det_inv;
    if (u < 0 || u > 1)
        return inter; // doesn't hi
    Vector3f qvec = crossProduct(tvec, e1);// qvec = Q
    ...
    if(u>=0 && v>=0 && u+v <= 1 && t_tmp >= 0){
        Vector3f coord = u * v0 + v * v1 + (1 - u - v) * v2;
        inter.coords =coord;
        inter.happened = true;
        inter.distance = fabs(dotProduct(coord - ray.origin,coord - ray.origin));
        inter.normal = normal;
        inter.m = m;
        inter.obj = this;
    }
